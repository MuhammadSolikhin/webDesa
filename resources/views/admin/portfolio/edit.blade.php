                    ...
                    <form method="post" action="{{ route('admin.portfolio.update', $portfolio) }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="category" name="category" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="filter-alam" {{ old('category', $portfolio->category) == 'filter-alam' ? 'selected' : '' }}>Alam</option>
                                <option value="filter-budaya" {{ old('category', $portfolio->category) == 'filter-budaya' ? 'selected' : '' }}>Budaya</option>
                                <option value="filter-kuliner" {{ old('category', $portfolio->category) == 'filter-kuliner' ? 'selected' : '' }}>Kuliner</option>
                            </select>
                        ...
                        <div>
                            ...
                            <div class="mt-2 mb-4 flex gap-2 flex-wrap">
                                @foreach((is_array($portfolio->image) ? $portfolio->image : [$portfolio->image]) as $img)
                                    <img src="{{ Storage::url($img) }}" class="h-32 object-cover rounded shadow-sm">
                                @endforeach
                            </div>
                            @endif
                            <input id="image" name="image[]" type="file" accept="image/*" multiple class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100" />
