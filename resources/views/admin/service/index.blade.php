<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.service.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                Tambah Layanan
            </a>
        </div>
    </x-slot>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white border border-gray-200">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $service->id }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $service->title }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ Str::limit($service->description, 50) }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 flex gap-2">
                                        ...
                                        <form action="{{ route('admin.service.destroy', $service) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin?');">
                                            @csrf
                                            @method('DELETE')
